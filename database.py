...
def migrate_makrolink(conn):
    ...
    if cols and "short_host" in cols:
        ...
        if _ml_host_brand == "makro":
            def _clean_ml_host(h):
                h = (h or "").strip().lower().replace("https://", "").replace("http://", "").strip("/")
                h = h.split("/")[0]
                if h.startswith("www."):
                    h = h[4:]
                return h
...
def migrate_biolink(conn):
    ...
    if "text_align" not in btn_cols:
        execute(conn, "ALTER TABLE biolink_buttons ADD COLUMN text_align TEXT NOT NULL DEFAULT 'left'")
    if uses_postgres():
        execute(
            conn,
            """
...
def init_mailing_schema(conn):
    ...
    try:
        ...
        print(f"⚠️  mail tenant schema: {exc}")
    conn.commit()
